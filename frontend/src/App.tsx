...
type SavedState = { parsedData: ParsedData; carrier: Carrier; phase: Phase; pickIdx: number; packIdx: number; pickChecks: boolean[]; packChecks: boolean[]; okihaiChecks: boolean[]; cautionChecks: boolean[]; flyerChecks: boolean[]; };
...
const COLOR_SWATCHES: Record<string, string> = { "ライトブルー":"#87CEEB","lblue":"#87CEEB","ブルー":"#2563EB","blue":"#2563EB","ピンク":"#F472B6","pink":"#F472B6","グレー":"#9CA3AF","gray":"#9CA3AF","ブラウン":"#92400E","brown":"#92400E","ブラック":"#52525B","black":"#52525B","インディゴ":"#4338CA","indigo":"#4338CA","ネイビー":"#1E3A5F","navy":"#1E3A5F","ホワイト":"#D1D5DB","white":"#D1D5DB" };
...
export default function App(){
  ...
  const pickItems=cd?.pickingItems??[];const packOrders=cd?.orders??[];
  ...
  const startPhase=(c:Carrier,tp:"picking"|"packing")=>{if(!parsedData||c==="")return;const d=c==="sagawa"?parsedData.sagawa:parsedData.yamato;setCarrier(c);setPickIdx(0);setPickChecks(new Array(d.pickingItems.length).fill(false));setPackIdx(0);setPackChecks(new Array(d.orders.length).fill(false));setOkihaiChecks(new Array(d.orders.length).fill(false));setCautionChecks(new Array(d.orders.length).fill(false));setFlyerChecks(new Array(d.orders.length).fill(false));setPhase(tp);};
  ...
  const pickChecked=pickChecks.filter(Boolean).length;const packChecked=packChecks.filter(Boolean).length;
