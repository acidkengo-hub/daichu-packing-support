...
const needsCautionSheet = (o: Order): boolean =>
  o.products.some(p => CAUTION_PREFIXES.some(pre => p.code.startsWith(pre)));
...
const getSwatchColor = (t: string): string | null => {
  if (COLOR_SWATCHES[t]) return COLOR_SWATCHES[t];
  for (const [k, v] of Object.entries(COLOR_SWATCHES)) { if (t.includes(k)) return v; }
  return null;
};
...
const STORE_CONFIG: Record<StoreKey, { label: string; color: string }> = {
  yahoo:   { label: "ヤフショ",  color: "#FF4444" },
  rakuten: { label: "楽天市場",  color: "#E6B422" },
  mercari: { label: "メルカリ",  color: "#00BFFF" },
  amazon:  { label: "Amazon",   color: "#FF9900" },
  other:   { label: "その他",    color: "#7e8085" },
};
function getStoreKey(shopName: string): StoreKey {
  if (shopName.includes("ヤフショ")) return "yahoo";
  if (shopName.includes("楽天")) return "rakuten";
  if (shopName.includes("メルカリ")) return "mercari";
  if (shopName.includes("Amazon")) return "amazon";
  return "other";
}
...
function loadCampaign(): CampaignSettings {
  try { const r = localStorage.getItem(CAMPAIGN_LS_KEY); return r ? { ...DEFAULT_CAMPAIGN, ...JSON.parse(r) } : DEFAULT_CAMPAIGN; } catch { return DEFAULT_CAMPAIGN; }
}
function saveCampaign(s: CampaignSettings) { try { localStorage.setItem(CAMPAIGN_LS_KEY, JSON.stringify(s)); } catch {} }

/** 佐川+ヤマトの全PickingItemをマージしてソート済みリストを返す */
function buildAllPickingItems(data: ParsedData): PickingItem[] {
  ...
  items.sort((a, b) => {
    const aM = a.code.startsWith("mercari-"), bM = b.code.startsWith("mercari-");
...
// ━━━ 共通スタイル ━━━
const S = {
  green: "var(--green)", black: "var(--black)", white: "var(--white)",
  grey: "var(--grey)", dim: "var(--dim)", s1: "var(--s1)", s2: "var(--s2)",
  s3: "var(--s3)", bd: "var(--border)", red: "var(--red)", amber: "var(--amber)",
  ease: "var(--ease)",
};
  ...
  return (
    <div className="flex flex-col gap-3.5 px-5 pt-4">
      <div className="flex items-center justify-between">
        ...
        <span className="font-[Roboto] font-extrabold text-[11px] tracking-[0.08em] uppercase px-3.5 py-[5px] rounded-full"
          style={{ background: bg, color: S.black }}>{label}</span>
        <button onClick={onBack} className="flex items-center gap-1 text-xs cursor-pointer bg-transparent border-none"
          style={{ color: S.grey }}>{Ic.aL(14, "#7e8085")}戻る</button>
      </div>
      <div className="h-[3px] overflow-hidden" style={{ background: S.s3 }}>
        <div className="h-full transition-[width] duration-400"
          style={{ background: S.green, width: `${(checked / total) * 100}%`, transitionTimingFunction: S.ease }} />
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex gap-3 mt-3.5">
      <button disabled={idx === 0} onClick={onPrev}
        ...
        style={{ background: S.s2, border: `1px solid ${S.bd}` }}>{Ic.aL(18)}前へ</button>
      {allDone
        ? <button onClick={onComplete}
            ...
            style={{ background: S.green, border: `1px solid ${S.green}`, color: S.black }}>完了{Ic.aR(18, "#000")}</button>
        : <button disabled={idx === total - 1} onClick={onNext}
            className="flex-1 flex items-center justify-center gap-1.5 py-[18px] rounded text-[15px] cursor-pointer transition-all active:scale-[0.97] disabled:opacity-20 disabled:cursor-default"
            style={{ background: S.s2, border: `1px solid ${S.bd}` }}>次へ{Ic.aR(18)}</button>}
    </div>
  );
}
...
function CheckButton({ done, label, onClick, disabled }: { done: boolean; label: string; onClick: () => void; disabled?: boolean }) {
  return (
    <button onClick={onClick} disabled={disabled}
      ...
      {done ? Ic.chk(20, "#000") : Ic.sq(20, "#fff")}{label}
    </button>
  );
}
...
function CheckStrip({ icon, color, borderColor, checked, onToggle, highlight, children }: {
  icon: React.ReactNode; color: string; borderColor: string; checked: boolean; onToggle: () => void; highlight?: boolean; children: React.ReactNode;
}) {
  return (
    <button onClick={onToggle}
      ...
      style={{ background: checked ? S.s1 : S.s2, borderLeft: `3px solid ${checked ? "#00fa27" : borderColor}`, borderTop: "none", borderRight: "none", borderBottom: "none", color: checked ? S.green : color, animation: highlight ? "pulse-warn 0.6s ease" : undefined }}>
      <span className="shrink-0 flex items-center justify-center w-[32px] h-[32px]" style={{ opacity: checked ? 1 : 0.7 }}>
...
function SwimwearBadge({ code }: { code: string }) {
  const l = SWIMWEAR_LABELS[code];
  if (!l) return null;
  return <span className="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-[Roboto] font-extrabold tracking-[0.04em]"
...
export default function App() {
  ...
  const cd: CarrierData | null = parsedData
    ? carrier === "sagawa" ? parsedData.sagawa : carrier === "yamato" ? parsedData.yamato : null
    : null;

  const pickItems = carrier === "all" && parsedData
    ? buildAllPickingItems(parsedData)
    : cd?.pickingItems ?? [];
  ...
  useEffect(() => {
    ...
    saveState({ parsedData, carrier, phase, pickIdx, packIdx, pickChecks, packChecks, okihaiChecks, cautionChecks, flyerChecks });
  }, [parsedData, carrier, phase, pickIdx, packIdx, pickChecks, packChecks, okihaiChecks, cautionChecks, flyerChecks]);
  ...
  const handleCSV = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (data.sagawa.orders.length + data.yamato.orders.length === 0) {
        setErr("注文データが見つかりません");
      } else {
        setParsedData(data); setCarrier(""); setPhase("home");
      }
    } catch (ex: unknown) {
  ...
  const startPhase = (c: Carrier, tp: "picking" | "packing") => {
    ...
    setCarrier(c);
    setPickIdx(0); setPackIdx(0);

    if (c === "all") {
      // 一括ピッキング: マージ後の件数でcheckを初期化
      const allItems = buildAllPickingItems(parsedData);
      setPickChecks(new Array(allItems.length).fill(false));
      setPackChecks([]); setOkihaiChecks([]); setCautionChecks([]); setFlyerChecks([]);
    } else {
      const d = c === "sagawa" ? parsedData.sagawa : parsedData.yamato;
      setPickChecks(new Array(d.pickingItems.length).fill(false));
      setPackChecks(new Array(d.orders.length).fill(false));
      setOkihaiChecks(new Array(d.orders.length).fill(false));
      setCautionChecks(new Array(d.orders.length).fill(false));
      setFlyerChecks(new Array(d.orders.length).fill(false));
    }
    setPhase(tp);
  };
  ...
  const handlePackComplete = useCallback((idx: number) => {
    ...
    if (!ok1 || !ok2 || !ok3) {
      setHighlightMissing(true); setTimeout(() => setHighlightMissing(false), 700); return;
    }
    togglePack(idx);
  ...
  useEffect(() => {
    ...
    if (curr.every(Boolean)) { setTimeout(() => setPhase("pickingSummary"), 400); }
    else if (j < curr.length - 1) { setTimeout(() => setPickIdx(j + 1), 300); }
  }, [pickChecks, phase]);
        ...
        <div className="flex-1 flex flex-col gap-6 px-5 pt-9 pb-6">
          {err && (
            <div className="flex items-center gap-2.5 px-4 py-3.5 text-sm"
              style={{ background: S.s2, borderLeft: `3px solid ${S.red}`, color: S.red }}>
          ...
          <div className="rounded overflow-hidden" style={{ background: S.s1, border: `1px solid ${S.bd}` }}>
            <div className="flex items-center gap-3 px-5 py-4" style={{ borderBottom: `1px solid ${S.bd}` }}>
              {Ic.file(18, "#00fa27")}
              <div>
                <p className="font-[Roboto] font-extrabold text-[11px] tracking-[0.12em] uppercase" style={{ color: S.green }}>STEP 1</p>
                <p className="text-base tracking-[-0.02em]">注文詳細CSVを読み込む</p>
              </div>
            </div>
            <div className="p-5">
              <label className="block rounded text-center cursor-pointer relative py-9 px-5"
                style={{ border: parsedData ? `2px solid ${S.green}` : `2px dashed ${S.bd}` }}>
                ...
                <div className="flex justify-center mb-3.5">
                  {uploading ? Ic.pkg(32, "#00fa27") : parsedData ? Ic.chkC(32, "#00fa27") : Ic.up(32, "#7e8085")}
                </div>
                <p className="text-sm" style={{ color: parsedData ? S.green : S.grey }}>
                  {uploading ? "読み込み中..." : parsedData ? "読み込み完了（タップで再読み込み）" : "CROSS MALL 注文詳細CSVをアップロード"}
                </p>
                {parsedData && (
                  <p className="text-xs mt-1.5" style={{ color: S.grey }}>
                    佐川 {parsedData.sagawa.orders.length}件 / ヤマト {parsedData.yamato.orders.length}件
                  </p>
                )}
              </label>
            </div>
          ...
          <div className="rounded overflow-hidden" style={{ background: S.s1, border: `1px solid ${S.bd}` }}>
            <button onClick={() => setShowSettings(!showSettings)}
              ...
              style={{ background: "transparent", border: "none", borderBottom: showSettings ? `1px solid ${S.bd}` : "none" }}>
              {Ic.gear(18, "#7e8085")}
              <div className="flex-1"><p className="text-[14px]">チラシ同封キャンペーン設定</p></div>
              <span className="text-[12px]" style={{ color: S.grey }}>{showSettings ? "閉じる" : "開く"}</span>
            </button>
            {showSettings && (
              <div className="p-4 flex flex-col gap-2">
                {(["yahoo", "rakuten", "mercari", "amazon"] as StoreKey[]).map(key => {
                  const conf = STORE_CONFIG[key]; const active = campaign[key];
                  return (
                    <button key={key}
                      onClick={() => { const next = { ...campaign, [key]: !active }; setCampaign(next); saveCampaign(next); }}
                      className="flex items-center gap-3 px-4 py-3 rounded cursor-pointer transition-all"
                      style={{ background: active ? `${conf.color}18` : S.s2, border: `1px solid ${active ? conf.color : S.bd}` }}>
                      <span className="w-3 h-3 rounded-full shrink-0" style={{ background: conf.color }} />
                      <p className="flex-1 text-[15px] text-left" style={{ color: active ? conf.color : S.grey }}>{conf.label}</p>
                      <span className="font-[Roboto] font-extrabold text-[12px]" style={{ color: active ? conf.color : S.grey }}>{active ? "ON" : "OFF"}</span>
                    </button>
                  );
                })}
              </div>
            ...
            <div className="flex flex-col gap-4">
              {([["sagawa", "佐川急便", parsedData.sagawa], ["yamato", "ヤマト運輸", parsedData.yamato]] as const).map(([key, label, data]) => (
                <div key={key} className="rounded overflow-hidden" style={{ background: S.s1, border: `1px solid ${S.bd}` }}>
                  <div className="flex items-center gap-3 px-5 py-4" style={{ borderBottom: `1px solid ${S.bd}` }}>
                    {key === "sagawa" ? Ic.truck(20, "#00fa27") : Ic.cat(20, "#fff")}
                    <div>
                      <p className="text-[17px]">{label}</p>
                      <p className="text-[12px] mt-0.5" style={{ color: S.grey }}>{data.orders.length}件</p>
                    </div>
                  </div>
                  <div className="p-4 flex gap-3">
                    <button onClick={() => startPhase(key, "picking")} disabled={data.pickingItems.length === 0}
                      className="flex-1 flex flex-col items-center gap-1.5 py-5 rounded cursor-pointer transition-all active:scale-[0.97] disabled:opacity-30 disabled:cursor-default"
                      style={{ background: S.s2, border: `2px solid ${S.bd}` }}>
                      {Ic.pkg(22, "#00fa27")}
                      <p className="text-[15px]">ピッキング</p>
                      <p className="text-[12px]" style={{ color: S.grey }}>{data.pickingItems.length}種 {data.totalPickingQty}点</p>
                    </button>
                    <button onClick={() => startPhase(key, "packing")} disabled={data.orders.length === 0}
                      className="flex-1 flex flex-col items-center gap-1.5 py-5 rounded cursor-pointer transition-all active:scale-[0.97] disabled:opacity-30 disabled:cursor-default"
                      style={{ background: S.s2, border: `2px solid ${S.bd}` }}>
                      {Ic.clip(22, "#00fa27")}
                      <p className="text-[15px]">梱包</p>
              ...
              {/* 全商品一括ピッキング */}
              <button onClick={() => startPhase("all", "picking")}
                disabled={parsedData.sagawa.pickingItems.length + parsedData.yamato.pickingItems.length === 0}
                className="w-full flex items-center justify-center gap-3 py-5 rounded cursor-pointer transition-all active:scale-[0.97] disabled:opacity-30 disabled:cursor-default"
                style={{ background: S.s2, border: `2px solid ${S.bd}` }}>
                {Ic.pkg(22, "#00fa27")}
                <div className="text-left">
                  <p className="text-[15px]">全商品一括ピッキング</p>
                  <p className="text-[12px]" style={{ color: S.grey }}>
                    佐川+ヤマト {parsedData.sagawa.totalPickingQty + parsedData.yamato.totalPickingQty}点
                  </p>
                </div>
              </button>
            </div>
      ...
      {phase === "picking" && pickItems.length > 0 && (() => {
        ...
        return <>
          ...
          <div className="mx-5 mt-3 flex items-center gap-3 px-5 py-4 rounded"
            style={{ background: ng ? "rgba(0,250,39,0.12)" : S.s2, border: `1px solid ${ng ? "rgba(0,250,39,0.3)" : S.bd}` }}>
            <p className="font-[Roboto] font-black text-[20px] tracking-[-0.3px]" style={{ color: S.green }}>{it.code}</p>
            <p className="font-[Roboto] font-extrabold text-[16px]" style={{ color: S.grey }}>{gp}/{gi.length}</p>
            ...
            <div className="flex flex-col justify-center rounded min-h-[280px] p-7 transition-all duration-300"
              ...
              <div className="flex items-center gap-2">
                ...
                <a href={getProductUrlForPicking(it.code, it.name)} target="_blank" rel="noopener noreferrer"
                  ...
                  onClick={e => e.stopPropagation()}>{Ic.link(18, "#7e8085")}</a>
              </div>
              ...
              <div className="mt-7 text-center shrink-0">
                ...
                {it.setSize > 0
                  ? <p className="font-[Roboto] font-extrabold text-[13px] mt-1.5" style={{ color: S.grey }}>
                      {it.setSize}枚セット × {it.qty / it.setSize}セット = {it.qty}枚
                    </p>
          ...
          <div className="px-5 pb-6">
            ...
            <NavButtons idx={pickIdx} total={pickItems.length} allDone={pickAllDone}
              onPrev={() => setPickIdx(i => i - 1)} onNext={() => setPickIdx(i => i + 1)}
          ...
          <div className="mt-auto pt-7 flex flex-col gap-3">
            {carrier !== "all" && (
              <button onClick={goToPacking}
                ...
                梱包フェーズへ{Ic.aR(18, "#000")}
              </button>
            )}
            <button onClick={goToHome}
              className="w-full flex items-center justify-center gap-2 py-5 rounded text-[15px] cursor-pointer transition-all active:scale-[0.97]"
              style={{ background: carrier === "all" ? S.green : S.s2, color: carrier === "all" ? S.black : S.white, border: carrier === "all" ? "none" : `1px solid ${S.bd}` }}>
              ホームに戻る{Ic.aR(18, carrier === "all" ? "#000" : "#fff")}
            </button>
          </div>
        </div>
      )}
      ...
      {phase === "packing" && packOrders.length > 0 && (() => {
        ...
        return <>
          ...
          <div className="flex-1 flex flex-col gap-3 p-5">
            {/* 店舗バッジ */}
            <div className="flex items-center gap-2.5 px-4 py-3 rounded"
              style={{ background: `${sc.color}18`, border: `1px solid ${sc.color}44` }}>
              <span className="w-3 h-3 rounded-full shrink-0" style={{ background: sc.color }} />
              <p className="font-[Roboto] font-extrabold text-[15px]" style={{ color: sc.color }}>{sc.label}</p>
              <p className="text-[12px]" style={{ color: S.grey }}>{o.shopName}</p>
            </div>
            ...
            <div className="rounded p-6 transition-all duration-300"
              style={{ background: S.s1, border: `2px solid ${done ? S.green : S.bd}`, borderLeft: `4px solid ${done ? S.green : sc.color}`, transitionTimingFunction: S.ease }}>
              ...
              {o.products.map((p, pi) => {
                const ca = needsColorEmphasis(p) ? (getSwatchColor(p.attr2) ? p.attr2 : getSwatchColor(p.attr1) ? p.attr1 : null) : null;
                const sw = ca ? getSwatchColor(ca) : null;
                const sa = sw ? (ca === p.attr1 ? p.attr2 : p.attr1) : null;
                  ...
                  <div key={pi} className="flex flex-col rounded mb-2 overflow-hidden" style={{ background: S.s2, border: `1px solid ${S.bd}` }}>
                    <div className="flex items-center gap-3 p-3.5">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <p className="font-[Roboto] font-black text-[20px] tracking-[-0.3px]" style={{ color: S.green }}>
                            {o.products.length > 1 ? `${pi + 1}. ` : ""}{p.code}
                          </p>
                          {pu && <a href={pu} target="_blank" rel="noopener noreferrer"
                            className="shrink-0 p-1 rounded transition-all active:scale-90"
                            style={{ background: S.s1, border: `1px solid ${S.bd}` }}
                            onClick={e => e.stopPropagation()}>{Ic.link(16, "#7e8085")}</a>}
                        </div>
                        ...
                        {sw && ca
                          ? <div className="flex items-center gap-2 mt-2">
                              <span className="inline-block w-[20px] h-[20px] rounded-full shrink-0"
                                style={{ background: sw, border: "2px solid rgba(255,255,255,0.3)", boxShadow: `0 0 8px ${sw}66` }} />
                              <p className="text-[20px] tracking-[-0.3px] font-[Roboto] font-extrabold" style={{ color: sw }}>{ca}</p>
                              {sa && <p className="text-[18px] ml-1" style={{ color: S.green }}>{sa}</p>}
                            </div>
                          : pa && <p className="text-[18px] mt-1.5" style={{ color: S.green }}>{pa}</p>}
                      </div>
                      <div className="shrink-0 text-right">
                        <p className="font-[Roboto] font-black text-[28px] tracking-[-0.5px]"
                    ...
                    {sw && ca && (
                      <div className="px-3.5 py-2 flex items-center gap-2 text-[13px]"
                        style={{ background: `${sw}18`, borderTop: `1px solid ${sw}44` }}>
                        <span className="w-3 h-3 rounded-full shrink-0" style={{ background: sw }} />
                        <span style={{ color: sw }}>カラー確認：{ca}</span>
                      </div>
                    )}
                  </div>
                );
          ...
          <div className="px-5 pb-6">
            ...
            <NavButtons idx={packIdx} total={packOrders.length} allDone={packAllDone}
              onPrev={() => setPackIdx(i => i - 1)} onNext={() => setPackIdx(i => i + 1)}
              onComplete={() => setPhase("packingSummary")} />
          </div>
        </>;
      })()}
      ...
      {phase === "packingSummary" && (
        <div className="flex-1 flex flex-col p-5">
          ...
          <div className="mt-auto pt-7 flex flex-col gap-3">
            <button onClick={goToHome}
              className="w-full flex items-center justify-center gap-2 py-5 rounded text-[15px] cursor-pointer transition-all active:scale-[0.97]"
              style={{ background: S.green, color: S.black }}>次の配送業者へ{Ic.aR(18, "#000")}</button>
            <button onClick={resetAll}
              ...
              style={{ background: S.s2, border: `1px solid ${S.bd}` }}>作業終了（データクリア）</button>
          </div>
        </div>
      )}
      ...
      {showCatalog && (
        <div className="fixed inset-0 z-50 flex flex-col" style={{ background: "rgba(0,0,0,0.95)" }}
          ...
          <div className="flex-1 overflow-auto p-2" onClick={e => e.stopPropagation()}>
            <img src="/daichu-packing-support/product-catalog.png" alt="商品一覧"
              className="w-full h-auto" style={{ maxWidth: "100%", touchAction: "pinch-zoom" }} />
          </div>
        </div>
      )}
    </div>
